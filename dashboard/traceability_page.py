...
def render_traceability_page():
    ...
    total=len(df); labeled=int(df["has_label"].sum()) if "has_label" in df.columns else 0
    fails=int((df.get("result")=="FAIL").sum()) if "result" in df.columns else 0
    c1,c2,c3=st.columns(3)
    c1.metric("Total records", total); c2.metric("Labeled", labeled); c3.metric("FAIL events", fails)
    with st.expander("Filters"):
        ...
        if model: df=df[df["model"].isin(model)]
        if res: df=df[df["result"].isin(res)]
    st.dataframe(df, width='stretch')
    st.download_button("Download CSV", df.to_csv(index=False).encode("utf-8"), "traceability.csv", "text/csv")
