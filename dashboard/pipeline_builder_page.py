...
def render_pipeline_builder():
    ...
    with st.expander("Post-processing"):
        ...
        topk=st.number_input("Top-K", 1, 200, 100)
    ...
    if st.button("Export pipeline.yaml"):
        cfg={"preprocess":{"resize":None if resize=='none' else int(resize),"normalize":bool(normalize),"denoise":bool(denoise)},
        ...
        with open(PIPELINE_PATH,"w",encoding="utf-8") as f: yaml.safe_dump(cfg,f,sort_keys=False)
