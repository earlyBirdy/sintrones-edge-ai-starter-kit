...
def render_fleet_page():
    ...
    devs = list_devices()
    try:
        st.dataframe(devs, width='stretch')
    except Exception:
        # If devs is not a DataFrame-like, Streamlit will still handle lists/dicts
        st.write(devs)
    ...
    with c1:
        if st.button("Heartbeat"):
            heartbeat(device); st.success("Heartbeat sent.")
    with c2:
        new_pack = st.text_input("Set Active Pack", "")
        if st.button("Apply Pack"):
            set_active_pack(device, new_pack); st.success("Pack set.")
    with c3:
        alarm = st.text_input("Raise Alarm", "")
        if st.button("Send Alarm"):
            raise_alarm(device, alarm); st.warning("Alarm raised.")
