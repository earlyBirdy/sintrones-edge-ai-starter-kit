import os, yaml, streamlit as st, time
...
def render_io_connectors():
    ...
    with st.expander("Add Camera"):
        ...
        if st.button("Add Camera"):
            cfg.setdefault("cameras", []).append({"name": name, "type": kind, "uri": uri})
            _save_cfg(cfg); st.success("Camera added")

    if cfg.get("cameras"):
        st.table(cfg["cameras"])
    ...
    modbus_port = st.number_input("Modbus/TCP port", min_value=1, max_value=65535, value=int(cfg.get("modbus",{}).get("port",502)))
    if st.button("Save PLC settings"):
        cfg["opcua"] = {"endpoint": opc_host}
        cfg["modbus"] = {"host": modbus_host, "port": int(modbus_port)}
        _save_cfg(cfg); st.success("Saved PLC settings")
    ...
    if st.button("Save MQTT"):
        cfg["mqtt"] = {"host": mqtt_host, "topic": mqtt_topic}
        _save_cfg(cfg); st.success("Saved MQTT")

    st.divider()
    st.markdown("#### Dry-run: emit a PASS/FAIL event (simulation)")
    unit = st.text_input("Unit ID", "U-0001")
    result = st.selectbox("Result", ["PASS","FAIL"])
    if st.button("Simulate Emit"):
        # For now write to logs/events.jsonl
        ...
        with open("logs/events.jsonl","a",encoding="utf-8") as f:
            f.write(yaml.safe_dump(line, explicit_start=False, explicit_end=False).replace(": null","") if False else (json.dumps(line)+"\n"))
