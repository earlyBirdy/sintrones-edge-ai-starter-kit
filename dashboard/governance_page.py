...
def render_governance():
    ...
    if not pack: st.info("No modelpack.yaml found."); return
    st.markdown("### Lineage")
    st.json({"model_id":pack.get("model_id"),"version":pack.get("version"),
             "train_data_commit":pack.get("train",{}).get("data_commit"),
             "metrics":pack.get("metrics",{}),"benchmarks":pack.get("benchmarks",{})})
    st.markdown("### Sign / Verify Artifacts")
    arts=pack.get("artifacts",{}); rows=[]
    for name, rel in (arts or {}).items():
        p=os.path.join(pack_dir,rel)
        rows.append({"name":name,"path":p,"exists":os.path.exists(p),"sha256":_sha256(p)})
    if rows: st.dataframe(rows, width='stretch')
    sig_path=os.path.join(pack_dir,"SIGNATURES.json")
    if st.button("Write SIGNATURES.json"):
        with open(sig_path,"w",encoding="utf-8") as f: json.dump(rows,f,indent=2)
