...
def promote_to_rule(record, roi=None, threshold=0.85, rule_dir=RULE_DIR):
    ...
    y={"id":f"rule_{record['id']}", "type":"roi_threshold" if roi else "threshold","value":float(threshold),
       "roi":roi,"source_image":record.get("path"),"created_at":time.strftime("%Y-%m-%dT%H:%M:%S")}
