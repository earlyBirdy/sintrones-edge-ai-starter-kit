...
def _compute_metrics(df: pd.DataFrame):
    total = len(df)
    # PASS/FAIL assumed from 'result' column; treat missing as PREDICT
    ...
    return dict(total=total, passes=passes, fails=fails, labeled=labeled, yield_pct=yield_pct, ppm=ppm)

def _pareto(df: pd.DataFrame):
    # Use 'defect' if available; fallback to 'label' categories starting with 'NG-'
    if "defect" in df.columns and df["defect"].notna().any():
        series = df["defect"].fillna("unknown")
    else:
        if "label" in df.columns:
            series = df["label"].fillna("OK").apply(lambda x: x if str(x).startswith("NG") else "OK")
        else:
...
def render_yield_dashboard():
    ...
    if df.empty:
        st.info("No trace data found. Put events/images under ./logs.")
        return
    ...
    c2.metric("PASS", m["passes"])
    c3.metric("FAIL", m["fails"])
    c4.metric("Yield %", f"{m['yield_pct']:.2f}%")
    c5.metric("DPPM (defects per million)", f"{m['ppm']:.0f}")

    # Trends (by day)
    if "day" in df.columns and df["day"].notna().any():
        trend = df.groupby("day").apply(lambda g: (g["result"]=="FAIL").sum()).reset_index(name="fails")
        trend["yield_pct"] = df.groupby("day").apply(
            lambda g: ((g["result"]=="PASS").sum() / max(1, ((g["result"]=="PASS").sum() + (g["result"]=="FAIL").sum()))) * 100.0
        ).values
        st.line_chart(trend.set_index("day")[["fails","yield_pct"]])  # streamlit will render lines with separate axes

    # Pareto of defects
    pareto = _pareto(df).head(15)
    if not pareto.empty:
        st.bar_chart(pareto.set_index("category")["count"])
    else:
        st.info("No defect categories found to plot Pareto.")
