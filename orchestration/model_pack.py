...
def save_model_pack(path, pack): 
    ...
    with open(os.path.join(path,"modelpack.yaml"),"w",encoding="utf-8") as f: yaml.safe_dump(pack, f, sort_keys=False)
...
def smoke_test(pack_dir):
    pack=load_model_pack(pack_dir); arts=pack.get("artifacts",{}); res={"artifacts":{}, "tests":{}}
    for k,v in arts.items():
        p=os.path.join(pack_dir,v); res["artifacts"][k]={"exists":os.path.exists(p),"sha256":_sha256(p) if os.path.exists(p) else ""}
    img=os.path.join(pack_dir,"tests","smoke.jpg"); res["tests"]["smoke.jpg"]={"exists":os.path.exists(img)}
    res["status"]="ok" if all(a["exists"] for a in res["artifacts"].values()) else "warn"; return res
...
def persist_bench_result(pack_dir, engine, size, fps, latency_ms):
    ...
    pack["benchmarks"][today][engine]={"fps":float(fps),"latency_ms":float(latency_ms),"size":str(size)}; save_model_pack(pack_dir, pack)
